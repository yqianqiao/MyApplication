...
/**
 *   Created by YX on 2019/5/20 14:13.
 */
class CalendarReminderUtils {
    ...
    private val CALENDARS_NAME = "boohee"
    private val CALENDARS_ACCOUNT_NAME = "BOOHEE@boohee.com"
    private val CALENDARS_ACCOUNT_TYPE = "com.android.boohee"
    private val CALENDARS_DISPLAY_NAME = "BOOHEE账户"
    ...
    private fun checkCalendarAccount(context: Context): Int {
        val userCursor = context.getContentResolver().query(Uri.parse(CALENDER_URL), null, null, null, null)
        try {
    ...
    private fun addCalendarAccount(context: Context): Long {
        ...
        val result = context.getContentResolver().insert(calendarUri, value)
     ...
     * 添加日历事件
     */
    fun addCalendarEvent(context: Context?, title: String, description: String, reminderTime: Long, previousDate: Int) {
        if (context == null) {
            return
        }
        ...
        val start = mCalendar.getTime().getTime()
        mCalendar.setTimeInMillis(start + 10 * 60 * 1000)//设置终止时间，开始时间加10分钟
        val end = mCalendar.getTime().getTime()
        ...
        val newEvent = context!!.getContentResolver().insert(Uri.parse(CALENDER_EVENT_URL), event)
            ?: //添加日历事件失败直接返回
            return //添加事件
        ...
        val uri = context!!.getContentResolver().insert(Uri.parse(CALENDER_REMINDER_URL), values)
            ?: //添加事件提醒失败直接返回
            return
    }
    ...
    fun deleteCalendarEvent(context: Context?, title: String) {
        ...
        val eventCursor = context!!.getContentResolver().query(Uri.parse(CALENDER_EVENT_URL), null, null, null, null)
        try {
            ...
            if (eventCursor!!.getCount() > 0) {
                ...
                while (!eventCursor!!.isAfterLast()) {
                    ...
                    if (!TextUtils.isEmpty(title) && title == eventTitle) {
                        ...
                        val rows = context!!.getContentResolver().delete(deleteUri, null, null)
                        if (rows == -1) { //事件删除失败
                            return
                        }
                    }
